---
name: debug-tests
description: Diagnose a failing test in this .NET automation template using the evidence-led loop (run manifest, Allure results, traces, logs, sanitized API/SQL evidence).
---

# Debug a failing test

This skill wraps the canonical evidence-led loop shared with Claude Code and human engineers. The
source of truth is [`docs/debugging.md`](../../../docs/debugging.md); also see [`AGENTS.md`](../../../AGENTS.md).
...
- Never claim a fix without verification; report blockers precisely.
- Never weaken, skip, retry, or delete a test to obtain green unless the owner approves a change in
  expected behaviour.
- Open a trace: `pwsh tests/Application.Tests/bin/Release/net10.0/playwright.ps1 show-trace <trace.zip>`.
